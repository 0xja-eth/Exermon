    ...
    public class CorrectionQuestion : BaseData {
		...
		public string[] words() {
            if (tepWords == null) {
                ...
                tepWords = temp.Split(new char[2] { '|', ' ' }, StringSplitOptions.RemoveEmptyEntries);
            }
            return tepWords;
        }
        ...
        /// </summary>
        /// <returns></returns>
        public int wid(int index) {
            string[] sen = sentences();
            int senid = sid(index);
            Debug.Log("bbbindex" + index);
            Debug.Log("bbbsenid" + senid);
            int wordNum = 0;
            for(int i = 1; i < senid; i++) {
                wordNum += words(i).Length;
                Debug.Log("bbbwordNum" + wordNum);
            }

            return index - wordNum + 1;
        }
        ...
        /// </summary>
        /// <returns></returns>
        public int sid(int index) {
            string[] sen = sentences();
            int sid = 1;
            int wordNum = sen[0].Split(new char[1] {' '}).Length;
            for (int i = 1; i < sen.Length; i++) {
                if (index < wordNum) break;
                wordNum += sen[i].Split(new char[1] { ' ' }).Length;
                sid++;
            }
            return sid;
        }

...
from game_module.models import GameConfigure, GroupConfigure
...
class BaseContainer(models.Model):
	...
	# 容器项数据（runtimes.py定义数据类型）
	# {
	# 'order': 容器项编号
	# 'ci_type': 容器项类型（枚举值）
	# }
	cont_items = jsonfield.JSONField(default=[], verbose_name="容器项数据")
...
class SlotContItem(BaseContItem):
	...
	# 所接受的装备项类（可多个）
	@classmethod
	def acceptedEquipItemClass(cls):
		return ()

	# 所接受的装备项属性名（可多个）
	@classmethod
	def acceptedEquipItemAttr(cls):
		return ()

	# 装备数目
	@classmethod
	def equipCount(cls):
		return len(cls.acceptedEquipItemClass())
